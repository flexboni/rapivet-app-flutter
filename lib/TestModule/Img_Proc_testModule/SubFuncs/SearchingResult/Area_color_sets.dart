...
class Area_color_sets {
  ...
  List<Color_int_rgb> area_colors;
  ...
  _get_av_rgb() {
    ...
    av_rgb = Color_int_rgb(  in_r: _av_r, in_g:_av_g, in_b:_av_b);
  }
  ...
  _get_av_lab(){
    ...
    av_lab= new Color_lab(in_l: lab_l, in_a: lab_a, in_b: lab_b) ;
  }

  print_forCheck() {
    print("@ "+area_name);
    if(is_using==false) {
      print("no used"); return;
    }
  ...
  get_vv(){
    ...
    for(int i=0; i<height; i++){
      for(int j=0; j<width; j++){
        ...
        vv_sum +=gap_r.toDouble()/255+gap_g.toDouble()/255+gap_b.toDouble()/255;
        vv_count++;
      }
    }
