  ...
  * */
  ...
  List<Area_color_sets> get_colorAreas_fromImg(imglib.Image img,
    ...
    for (int list_i = 0; list_i < pos_list.length; list_i++) {
      ...
      List<Color_int_rgb> this_area_colors_list = [];
      ...
      for (int p = y - r; p <= y + r; p++) {
        for (int t = x - r; t <= x + r; t++) {
          ...
          Color_int_rgb color_rgb = Color_int_rgb(in_r: r, in_g: g, in_b: b);
  ...
  _get_lab_from_rgb(Color_int_rgb rgb) {
    return Lab_manager()
        .rgb2lab(rgb.r, rgb.g, rgb.b, is_directTo_Color_lab: true);
  }
}
