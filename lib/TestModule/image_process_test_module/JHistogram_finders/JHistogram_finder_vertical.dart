...
import '../../../../common/JPosition.dart';
import '../../../../common/MakeOrder_inList.dart';
...
class JHistogram_finder_vertical {
  ...
  find_horizontal_area(Uint8List bytesIn, int height, int width,
      List<JArea> vertical_maxArea_list, int target_area_index) {
    // 샘플에어리어 버티컬 라인 0번째를 먼저 너며줌.
    try {
      List<int> XareaPoints = _operate_find_horizontal_area(
          bytesIn, height, width, vertical_maxArea_list, target_area_index);
      return XareaPoints;
    } catch (e) {
      return null;
    }
  ...
  _operate_find_horizontal_area(Uint8List bytesIn, int height, int width,
      List<JArea> vertical_maxArea_list, int area_index) {
    int searching_y = ((vertical_maxArea_list[area_index].end_pos -
                vertical_maxArea_list[area_index].start_pos) *
            0.208) //0.48, 해당값으로 민감도 조정 가능함.
        .toInt();
    int rec_side_width = vertical_maxArea_list[area_index].end_pos -
        vertical_maxArea_list[area_index].start_pos;
  ...
  find_lastRec_in_stick(Uint8List bytesIn, List<JArea> JAreaList_ys,
      List<int> XareaPoints, int height, int width) {
    int y1 = JAreaList_ys[1].end_pos;
    int y2 = JAreaList_ys[2].start_pos;
  ...
  Uint8List check_ultimate_area(Uint8List bytesIn, int height, int width,
    ...
    for (int j = 0; j < width; j++) {
      for (int t = 0; t < JAreaList_ys.length; t++) {
        int start_pos =
            (JAreaList_ys[t].start_pos + (oneArea_width * 0.5 * 0.62)).toInt();
        int end_pos =
            (JAreaList_ys[t].end_pos - (oneArea_width * 0.5 * 0.62)).toInt();
