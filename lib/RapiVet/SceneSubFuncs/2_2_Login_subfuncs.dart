...
class Login_subFuncs{
  ...
  operate_after_login(BuildContext context, String token, {bool is_simple_login = false}) async{
    ...
    if(rapivetStatics.pet_data_list== [] || rapivetStatics.pet_data_list==null || rapivetStatics.pet_data_list.length==0){
      JToast().show_toast("Faça o cadastramento do primeiro pet.", false);
      Navigator.pushReplacement(
          context,
          MaterialPageRoute(
              builder: (BuildContext context) => RegisterPet_scene(
                  COME_FROM.WELCOME, PET_REGISTER_MODE.ADD)));
    }else{
