...
class Detail_Check_stick {
  ...
  int get_stick_compensate(
    ...
    * */
    ...
    List results1 = _get_exact_x_point(StickArea_pointsX[2], scale,
        stick_area_h, stick_centerY, height, width, bytesNew);
    print("!!!!!!!!!!!!!!!!!!");
    List results2 = _get_exact_x_point(StickArea_pointsX[5], scale,
        stick_area_h, stick_centerY, height, width, bytesNew);
  ...
  // stick x 다시 조정
  List _rearrange_stickXs(int new_stick_x_0, int new_stick_x_5, double scale,
      List<int> StickArea_pointsX, Uint8List bytesNew, int height, int width) {
    ...
    for (int i = 0; i < new_stickArea_pointsX.length; i++) {
      new_stickArea_pointsX_original
          .add((new_stickArea_pointsX[i] / scale).round());
    }
