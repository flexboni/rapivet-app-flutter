...
class JHistogram_finder_horizontal {
  ...
  _last_creditTest(List<JArea> maxArea_list, int average_width) {
    int point1 =
        ((maxArea_list[1].end_pos + maxArea_list[1].start_pos) / 2).toInt();
    int point2 =
        ((maxArea_list[2].end_pos + maxArea_list[2].start_pos) / 2).toInt();

    if (point2 - point1 > average_width * 1.5)
      return true;
    else
      return false;
  }
  ...
  Uint8List remove_stickArea_and_bg(Uint8List byteIn, int height, int width,
    ...
    for (int j = 0; j < width; j++) {
      for (int t = targetSample_index; t < targetSample_index + 1; t++) {
        ...
        for (int i = (maxAreaList[t].start_pos + gap * 0.2).toInt();
            i <= maxAreaList[t].end_pos;
            i++) {
          int index = i * width + j;
        ...
        for (int i = maxAreaList[t].start_pos;
            i <= (maxAreaList[t].start_pos + gap * 0.2).toInt();
            i++) {
